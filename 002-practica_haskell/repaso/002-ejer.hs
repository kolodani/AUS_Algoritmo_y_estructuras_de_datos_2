...
oblongoNumber :: [Int]
oblongoNumber = [y | x <- [1 ..], y <- [1 .. (x * (x + 1))], (x * (x + 1)) == y]
...
abundantes :: [Integer]
abundantes = [x | x <- [1 ..], x < sum [a | a <- [2 .. (x - 1)], mod x a == 0]]
...
euler :: Int -> Int
euler x = sum [3, 6 .. (x - 1)] + sum [5, 10 .. (x - 1)]

euler2 :: Int -> Int
euler2 n = sum [x | x <- [1 .. (n - 1)], mod x 3 == 0 || mod x 5 == 0]
